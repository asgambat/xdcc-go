...
func (c *Client) Search(opts *SearchOptions) (*SearchResult, error) {
	...
	if len(opts.Ext) > 0 {
		q += "&ext=" + strings.Join(opts.Ext, ",")
	}
	if opts.Compact {
		q += "&compact=true"
	}
	if opts.Page > 0 {
		q += fmt.Sprintf("&page=%d", opts.Page)
	}
	if opts.PageSize > 0 {
		q += fmt.Sprintf("&pageSize=%d", opts.PageSize)
	}

	url := "/api/search?" + q

	var result SearchResult
	if err := c.doJSON("GET", url, nil, &result); err != nil {
